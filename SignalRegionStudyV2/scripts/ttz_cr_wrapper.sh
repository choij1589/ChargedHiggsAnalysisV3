...
#   validate          $ERA
#   datacard          $ERA
#   combine_era       $OUTPUT_ERA  (input eras read from constants below)
#   gof_data          $ERA
...
#   plotpostfit       $ERA
#
# All steps assume the CR five-segment templates layout:
...
# inside SignalRegionStudyV2 so cvmfs+CMSSW (incl. pyROOT) is on the worker's
...
case "$STEP" in
    ...
    datacard)
        ...
        ;;
    combine_era)
        # ERA arg is the OUTPUT era for combine_era (Run2 / Run3 / All).
        case "$ERA" in
            Run2) IN_ERAS="$ERAS_RUN2" ;;
            Run3) IN_ERAS="$ERAS_RUN3" ;;
            All)  IN_ERAS="Run2,Run3" ;;
            *) echo "ERROR: combine_era ERA must be Run2, Run3, or All (got: $ERA)"; exit 1 ;;
        esac
        python3 python/combineDatacards.py --mode era \
            --eras "$IN_ERAS" --channel "$CHANNEL" --masspoint "$MASSPOINT" \
            --method "$METHOD" --binning "$BINNING_TAG" --output-era "$ERA"
        # Build workspace.root here so the parallel gof_data / gof_toys / fitdiag
        # children don't race on text2workspace.py.
        TPL_DIR="${WORKDIR}/SignalRegionStudyV2/templates/${ERA}/${CHANNEL}/${MASSPOINT}/${METHOD}/${BINNING_TAG}"
    ...
    *)
        ...
        echo "Valid: template validate datacard combine_era gof_data gof_toys gof_collect fitdiag plotpostfit"
        exit 1
        ;;
esac
