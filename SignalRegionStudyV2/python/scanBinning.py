...
def main():
    ...
    # Load sample groups
    with open(f"{workdir}/SignalRegionStudyV2/configs/samplegroups.json") as f:
        ...
        for era in eras:
            for channel in channels:
                for mp in low_ma_masspoints:
                    basedir = f"{workdir}/SignalRegionStudyV2/samples/{era}/{channel}/{mp}"
    ...
    for era, channel, masspoint in scan_cases:
        basedir = f"{workdir}/SignalRegionStudyV2/samples/{era}/{channel}/{masspoint}"

        # Load fit parameters
        # Try to load from existing templates first
        fit_path = f"{workdir}/SignalRegionStudyV2/templates/{era}/SR1E2Mu/{masspoint}/Baseline/extended/signal_fit.json"
        if not os.path.exists(fit_path):
            # For SR3Mu, also check SR1E2Mu
            fit_path = f"{workdir}/SignalRegionStudyV2/templates/{era}/{channel}/{masspoint}/Baseline/extended/signal_fit.json"
        if not os.path.exists(fit_path):
