...
parser.add_argument("--mhc", type=int, default=160, help="Plot only this MHc value (Baseline only)")
...
# HEPData curves are quoted in B_sig units, so they are only meaningful for --mode BR.
...
# HEPData curves are in B_sig units; xsec mode skips the overlay.
...
if args.method == "Baseline":
    ...
    if args.compare_mhc:
        ...
        for mhc, g in graphs_compare:
            leg.AddEntry(g['exp'], f"m_{{H^{{#pm}}}} = {mhc} GeV", "lp")

        print(f"Created MHc-comparison plot ({len(graphs_compare)} MHc values, Baseline)")
    else:
        limits = _filter_by_mhc(limits, args.mhc)
        if not limits:
            raise RuntimeError(f"No mass points found for MHc{args.mhc} in JSON")
        ...
        mhc_label_txt.DrawLatex(0.20, 0.72, f"m_{{H^{{#pm}}}} = {args.mhc} GeV")
        ...
        print(f"Created Brazilian plot with {len(limits)} mass points (Baseline, MHc{args.mhc})")
