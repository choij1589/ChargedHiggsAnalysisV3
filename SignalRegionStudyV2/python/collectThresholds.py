...
def load_masspoints(workdir):
    path = f"{workdir}/SignalRegionStudyV2/configs/masspoints.json"
    with open(path) as f:
        cfg = json.load(f)
...
def collect_one(workdir, masspoint, binning):
    ...
    for era in ERAS:
        ...
        for channel in CHANNELS:
            tdir = f"{workdir}/SignalRegionStudyV2/templates/{era}/{channel}/{masspoint}/ParticleNet/{binning}"
...
def main():
    ...
    for masspoint in masspoints:
        ...
        out_path = f"{workdir}/SignalRegionStudyV2/configs/thresholds/{masspoint}.json"
