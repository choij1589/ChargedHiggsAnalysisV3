...
for era in era_list:
    for channel in channel_list:
        template_dirs.append({
            "era": era,
            "channel": channel,
            "path": f"{WORKDIR}/SignalRegionStudyV2/templates/{era}/{channel}/{args.masspoint}/{args.method}/{binning_suffix}"
        })
...
VALIDATION_DIR = f"{WORKDIR}/SignalRegionStudyV2/templates/{args.era}/{args.channel}/{args.masspoint}/{args.method}/{binning_suffix}/validation"
...
def load_systematics_config():
    ...
    if len(era_list) == 1 and len(channel_list) == 1:
        ...
        config_path = f"{WORKDIR}/SignalRegionStudyV2/configs/systematics.{era_list[0]}.json"
