...
This file is the Codex-facing project guide for `SignalRegionStudyV2`. It is
checked against the live code and configs; use `docs/*` for deeper background
on the statistical procedures and review history.

## Project Scope

`SignalRegionStudyV2` is the current baseline for charged Higgs signal-region
analysis and limit extraction. It covers preprocessing, binned template
production, datacard generation, channel/era combination, HiggsCombine limit
...
For `automize/makeBinnedTemplates.sh`, DAG dependencies are: channel templates,
datacards, validation, channel combination, per-era asymptotics, era
combination, then combined asymptotics. Use `--start-from combine_era` only when
per-era artifacts already exist.
...
- `baseline`: full Baseline run, 61 mass points.
- `particlenet`: ParticleNet run, 3 trained points.
- `limits`: curated Baseline plotting list, 28 mass points.
- `partial_unblind`: partial-unblind subset.
- `impact.baseline`, `impact.particlenet`: impact-plot subsets.
- `signal_injection.baseline`, `signal_injection.particlenet`: injection subsets.
- `hybridnew.baseline`, `hybridnew.particlenet`: HybridNew comparison subsets.
- `gof.baseline`, `gof.particlenet`: Goodness-of-Fit subsets.
- `unblind.baseline`, `unblind.particlenet`: mass points used by `--unblind`
  automation. As of the current config, `unblind.particlenet` is empty, so
  `automize/* --method ParticleNet --unblind` selects no mass points unless the
  config is updated.
...
Default binning is `extended`: 17 bins with +/-10 sigma coverage, a 15-bin
uniform core from -5 sigma to +5 sigma, and two sideband bins.

Signal A-mass fits use an unbinned Double Crystal Ball fit in
`python/makeBinnedTemplates.py` and write `signal_fit.json` plus
`signal_fit.png`. SR1E2Mu and SR3Mu are fit independently.
...
- `WORKDIR not set`: source module-local `setup.sh` in `SignalRegionStudyV2`.
