...
Aggregated ParticleNet background weights and optimized score thresholds, one
file per ParticleNet masspoint (3 total). Generated by
`python/collectThresholds.py` (or the shell wrapper `scripts/collectThresholds.sh`)
from the per-template JSONs produced by `makeBinnedTemplates.py`. Run once after
templates for all 8 eras × 2 channels are available.

Schema (`era → channel → entry`):
```json
{
  ...
  "2018": {
    "SR1E2Mu": {
      "mass_window": [82.10, 97.92],
      "weights": {"nonprompt": 0.46, "diboson": 0.19, "ttX": 0.35},
      "yields":  {"nonprompt": 30.65, "diboson": 12.60, "ttX": 23.75},
      "total_yield": 67.00,
      "threshold": 0.68,
      "sensitivity": {"initial": 0.80, "max": 1.05, "improvement": 0.31}
    },
    "SR3Mu": { ... }
  }
}
```
Missing (era, channel) entries (no templates yet) appear as `null`.
Per-template `background_weights.json` / `threshold.json` remain authoritative
for current consumers; this file is for inspection and future migration.
