...
def get_trig_prefix(ptcorr, measure):
...
def get_hist(sample, ptcorr, abseta, wp, syst="Central"):
    ...
    trig_prefix = get_trig_prefix(ptcorr, args.measure)
    ...
    try:
        if syst == "Stat":
            h = f.Get(f"{prefix}/QCDEnriched/{trig_prefix}/{wp}/Central/MT"); h.SetDirectory(0)
        else:
            h = f.Get(f"{prefix}/QCDEnriched/{trig_prefix}/{wp}/{syst}/MT"); h.SetDirectory(0)
        f.Close()
        return h
    except:
        logging.debug(f"Cannot find {prefix}/QCDEnriched/{trig_prefix}/{wp}/{syst}/MT for sample {sample}")
        return None
...
if __name__ == "__main__":
    ...
    for sample, wp in product(DATAPERIODs, ["loose", "tight"]):
        ...
        json_path = f"results/{args.era}/JSON/{args.measure}/{subdir}{sample}_{wp}.json"
