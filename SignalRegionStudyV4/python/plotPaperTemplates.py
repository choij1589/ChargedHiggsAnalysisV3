...
def parse_args():
    ...
    parser.add_argument("--masspoint", default="MHc130_MA90",
                        help="mass point, or 'all' for every available fitdiag")
    ...
    parser.add_argument("--signal-source", dest="signal_source",
                        ...
                        help="template signal source (V4 production: interp-signal)")
    parser.add_argument("--output-root", default="results/plots/paper/templates",
                        help="base output directory, relative to SignalRegionStudyV4 "
                             "unless absolute")
...
def discover_masspoints(method, signal_source):
    """Mass points with an All/Combined fitDiagnostics file.

    V4 layout puts the mass point first and the binning nowhere, so the scan
    is over templates/*/{method}/{source}/All/Combined rather than V3's
    templates/All/Combined/*/{method}/{binning}_unblind.
    """
    base = Path(srspaths.module_dir()) / "templates"
    found = []
    for masspoint_dir in sorted(base.iterdir()):
        if not masspoint_dir.is_dir():
            continue
        masspoint = masspoint_dir.name
        fitdiag = Path(srspaths.template_dir(
            masspoint, method, FIT_ERA, FIT_CHANNEL, source=signal_source)
        ) / "combine_output" / "fitdiag" / f"fitDiagnostics.{masspoint}.{method}.root"
        if fitdiag.exists():
            found.append(masspoint)
    if not found:
        raise FileNotFoundError(
            f"No fitDiagnostics files found under {base} for method={method}, "
            f"signal_source={signal_source}")
    return found


def setup_module(masspoint, method, signal_source, debug):
...
def draw_panel(out_path, era_scope, channel_scope, fit_stage,
    ...
    if show_signal:
        plotter.canv.cd(1)
        # Heavy line, no fill, matching plotPaperLRModified.py.
        ...
        signal_legend.AddEntry(signal, pfm.masspoint_label(masspoint),
                               SIGNAL_LEGEND_OPT)
        signal_legend.Draw()
...
def draw_masspoint(masspoint, method, signal_source, output_root, ratio_max, debug):
    setup_module(masspoint, method, signal_source, debug)
...
def main():
    ...
    masspoints = (discover_masspoints(args.method, args.signal_source)
                  if args.masspoint == "all" else [args.masspoint])
    logging.info("Mass points: %s", ", ".join(masspoints))

    total = 0
    for masspoint in masspoints:
        total += len(draw_masspoint(masspoint, args.method, args.signal_source,
