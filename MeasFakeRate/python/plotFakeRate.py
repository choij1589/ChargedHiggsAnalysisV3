...
def setHistStyle(projections, rate_syst=0.3):
    for projection in projections.values():
        ...
        projection.GetXaxis().SetRangeUser(10., 100.)
...
if args.isQCD:
    file_path = f"{WORKDIR}/MeasFakeRate/results/{args.era}/ROOT/{args.measure}/fakerate_qcd.root"
else:
    file_path = f"{WORKDIR}/MeasFakeRate/results/{args.era}/ROOT/{args.measure}/fakerate.root"
...
canvas = CMS.cmsCanvas("", ptcorr_bins[0], ptcorr_bins[-1], 
                             0., 1., 
                             "p_{T}^{corr}", 
                             title, 
                             square=False, 
