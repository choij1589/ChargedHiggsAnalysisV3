...
parser.add_argument("--hltpath", type=str, default="Mu8El23", help="HLT path")
...
output_dir = f"plots/{args.era}/{args.hltpath}_{args.object}"
...
def create_eta_projections():
    ...
    for pt_bin in range(1, n_pt_bins + 1):
        ...
        canvas = CMS.cmsDiCanvas(f"c_eta_ptbin{pt_bin}", xmin, xmax, 0.0, 1.1, 0.8, 1.2, 
...
def create_pt_projections():
    ...
    for eta_bin in range(1, n_eta_bins + 1):
        ...
        # Create CMS-style canvas with two pads (using log scale for pt)
        canvas = CMS.cmsDiCanvas(f"c_pt_etabin{eta_bin}", xmin, xmax, 0.0, 1.1, 0.8, 1.2, 
...
def create_eta_summary():
    ...
    for eta_bin in range(1, n_eta_bins + 1):
        ...
        if len(data_effs) > 0:
            ...
            avg_data_eff = avg_sim_eff = 0.0
            avg_data_err = avg_sim_err = 0.0
    ...
    # Create CMS-style canvas with two pads
    canvas = CMS.cmsDiCanvas("c_eta_summary", xmin, xmax, 0.0, 1.1, 0.8, 1.2, 
                            "|#eta|", "Efficiency", "Data/MC", square=True, iPos=0, extraSpace=0)
...
def create_pt_summary():
    ...
    canvas = CMS.cmsDiCanvas("c_pt_summary", xmin, xmax, 0.0, 1.1, 0.8, 1.2, 
                            "p_{T} [GeV]", "Efficiency", "Data/MC", square=True, iPos=0, extraSpace=0)
    ...
    title_label.SetTextFont(42)
    title_label.SetTextSize(0.04)
    title_label.DrawLatex(0.7, 0.55, "|#eta|-averaged")
...
def create_multi_eta_pt_projections():
    ...
    # Collect all eta-bins (no gap bins for muons, but may have gaps for electrons)
    valid_eta_bins = [eta_bin for eta_bin in range(1, n_eta_bins + 1)]
    n_valid_bins = len(valid_eta_bins)

    # Get color palette - use PALETTE for EMu eta bins
    ...
    for i, eta_bin in enumerate(valid_eta_bins):
        ...
        h_sim_proj = h_sim.ProjectionY(f"sim_pt_multi_eta{eta_bin}", eta_bin, eta_bin)

        # Create scale factor histogram (data/MC ratio)
        h_sf_proj = h_data_proj.Clone(f"sf_pt_multi_eta{eta_bin}")
        h_sf_proj.Divide(h_sim_proj)
        clean_nan_values(h_sf_proj)  # Clean scale factor - fill with 0
        ...
        # Use appropriate eta label based on object type
        if args.object == "electron":
...
def create_multi_eta_summary():
    ...
    for i, eta_bin in enumerate(valid_eta_bins):
        ...
        for pt_bin in range(1, n_pt_bins + 1):
            ...
            if (data_eff > 0 and sim_eff > 0 and not math.isnan(data_eff) and not math.isnan(sim_eff)):
                data_effs.append(data_eff)
                data_errs.append(data_err)
                sim_effs.append(sim_eff)
                sim_errs.append(sim_err)
        ...
        if len(data_effs) > 0:
            ...
            # Calculate scale factor as data/sim ratio
            avg_sf_eff = avg_data_eff / avg_sim_eff if avg_sim_eff > 0 else 0.0
            # Approximate scale factor error (simplified)
            avg_sf_err = avg_data_err / avg_sim_eff if avg_sim_eff > 0 else 0.0
        else:
    ...
    # Create CMS-style canvas with two pads
    eta_axis_label = "|#eta_{SC}|" if args.object == "electron" else "|#eta|"
    canvas = CMS.cmsDiCanvas("c_multi_eta_summary", xmin, xmax, 0.0, 1.1, 0.5, 1.5,
                            eta_axis_label, "Efficiency", "Scale Factor", square=True, iPos=0, extraSpace=0)
