...
f = ROOT.TFile.Open(f"results/{args.era}/ROOT/efficiency_EleID.root")
...
# Define gap regions to skip (eta bins 3 and 8)
gap_bins = [3, 8]

# Create output directory
output_dir = f"plots/{args.era}/EleID"
os.makedirs(output_dir, exist_ok=True)

def create_eta_projections():
    """Create 1D eta projections for each pt bin with scale factor panel, skipping gap regions"""
    ...
    for pt_bin in range(1, n_pt_bins + 1):
        ...
        # Set gap bins to zero
        for gap_bin in gap_bins:
            h_data_proj.SetBinContent(gap_bin, 0)
            h_data_proj.SetBinError(gap_bin, 0)
            h_sim_proj.SetBinContent(gap_bin, 0)
            h_sim_proj.SetBinError(gap_bin, 0)
            h_sf.SetBinContent(gap_bin, 0)
            h_sf.SetBinError(gap_bin, 0)
        ...
        canvas = CMS.cmsDiCanvas(f"c_eta_ptbin{pt_bin}", xmin, xmax, 0.0, 1.1, 0.5, 1.5, 
                                "#eta_{SC}", "Efficiency", "Data/MC", square=True, iPos=0, extraSpace=0)
...
def create_pt_projections():
    """Create 1D pt projections for each eta bin with scale factor panel, skipping gap regions"""
    ...
    for eta_bin in range(1, n_eta_bins + 1):
        ...
        # Save canvas
        canvas.SaveAs(f"{output_dir}/eff_vs_pt_etabin{eta_bin}_{format(eta_low, '.3f').replace('.', 'p').replace('-','m')}{format(eta_high, '.3f').replace('.', 'p').replace('-','m')}.png")

def create_eta_summary():
    """Create eta summary plot with pt-averaged efficiencies, skipping gap regions"""
    ...
    for eta_bin in range(1, n_eta_bins + 1):
        # Skip gap regions
        if eta_bin in gap_bins:
            h_data_eta.SetBinContent(eta_bin, 0)
            h_data_eta.SetBinError(eta_bin, 0)
            h_sim_eta.SetBinContent(eta_bin, 0)
            h_sim_eta.SetBinError(eta_bin, 0)
            h_sf_eta.SetBinContent(eta_bin, 0)
            h_sf_eta.SetBinError(eta_bin, 0)
            continue
...
def create_pt_summary():
    """Create pt summary plot with eta-averaged efficiencies, skipping gap regions"""
    ...
    title_label.SetTextFont(42)
    title_label.SetTextSize(0.04)
    title_label.DrawLatex(0.7, 0.45, "#eta_{SC}-averaged")
...
def create_multi_eta_pt_projections():
    ...
    # Collect eta-bins excluding gaps
    valid_eta_bins = [eta_bin for eta_bin in range(1, n_eta_bins + 1) if eta_bin not in gap_bins]
    n_valid_bins = len(valid_eta_bins)

    # Get color palette - use PALETTE_LONG for 8 eta bins
    colors = PALETTE_LONG[:n_valid_bins]
    ...
    # Create projections for each valid eta bin
    for i, eta_bin in enumerate(valid_eta_bins):
        ...
        eta_ranges.append(f"{eta_low:.3f} < #eta_{{SC}} < {eta_high:.3f}")
    ...
    # Create legend - position in upper right to avoid data overlap
...
def create_multi_eta_summary():
    ...
    canvas = CMS.cmsDiCanvas("c_multi_eta_summary", xmin, xmax, 0.0, 1.1, 0.5, 1.5,
                            "#eta_{SC}", "Efficiency", "Scale Factor", square=True, iPos=0, extraSpace=0)
