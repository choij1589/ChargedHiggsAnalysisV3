#!/usr/bin/env python
"""
Genetic Algorithm optimization launcher for ParticleNetMD hyperparameter tuning.

Uses shared memory datasets to enable efficient parallel training of multiple
model configurations with DisCo loss for mass decorrelation.

Key features for ParticleNetMD:
- DisCo loss with fixed disco_lambda
- 16 KS tests for overfitting detection
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Launch GA hyperparameter optimization for ParticleNetMD")
...
def run_training_population(config, args, population, iteration, base_dir):
    """Launch parallel training using shared memory datasets with DisCo loss."""
    ...
    disco_params = config.get_disco_parameters()
...
def run_overfitting_check(config, args, ga_module, iteration, base_dir):
    """Run overfitting detection with 16 KS tests and filter population."""
    logging.info("=" * 60)
    logging.info("OVERFITTING DETECTION & FILTERING (16 KS Tests)")
    logging.info("=" * 60)

    # Pass actual population size to match trained models
    overfitted_indices = evaluateGAModels.evaluate_iteration(
        config, args.signal, args.channel, args.device, iteration, base_dir,
        pilot=args.pilot, population_size=len(ga_module.population)
    )

    original_size = len(ga_module.population)
    removed = ga_module.filterPopulation(overfitted_indices)

    logging.info(f"Filtered: {removed}/{original_size} overfitted models")
...
def update_population_fitness(config, ga_module, args, iteration, base_dir):
    ...
    if penalty_weight > 0:
        logging.info(f"Using overfitting penalty: lambda = {penalty_weight}")
...
def main():
    """Main GA optimization workflow with DisCo loss."""
    ...
    logging.info("Loading GA configuration for ParticleNetMD...")
    ...
    nNodes, optimizer, initLR, weight_decay, scheduler = best['chromosome']
    disco_params = config.get_disco_parameters()

    logging.info("\nBest Hyperparameters:")
    logging.info(f"  nNodes: {nNodes}")
    logging.info(f"  optimizer: {optimizer}")
    logging.info(f"  initLR: {initLR}")
    logging.info(f"  weight_decay: {weight_decay}")
    logging.info(f"  scheduler: {scheduler}")
    logging.info(f"  disco_lambda: {disco_params['disco_lambda']} (fixed)")
