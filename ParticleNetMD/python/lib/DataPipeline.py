...
class DataPipeline:
    ...
    def create_datasets(self) -> Tuple[List[Any], List[Any], List[Any]]:
        ...
        use_subsampling = (max_events is not None) or self.config.args.pilot
        ...
        try:
            if self.config.use_groups:
                ...
                if use_subsampling:
                    ...
                    # Pilot mode: single train fold, reduced event caps per class
    ...
    def inject_loaders(self, train_loader, valid_loader, test_loader) -> None:
        """Accept pre-created DataLoaders, bypassing create_datasets/create_data_loaders.

        Used by lambda sweep workers that build loaders from shared-memory batches
        instead of loading data from disk.
        """
        self.train_loader = train_loader
        self.valid_loader = valid_loader
        self.test_loader = test_loader
        self.train_data = [None] * len(train_loader.dataset)
        self.valid_data = [None] * len(valid_loader.dataset)
        self.test_data = [None] * len(test_loader.dataset)
