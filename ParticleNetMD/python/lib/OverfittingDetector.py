...
class OverfittingDetector:
    ...
    def _get_predictions(self, model, data_list: List, batch_size: int = 1024) -> Dict[str, np.ndarray]:
        """Get model predictions on dataset using batched inference to avoid OOM."""
        from torch_geometric.loader import DataLoader
        from torch_geometric.data import Batch

        loader = DataLoader(data_list, batch_size=batch_size, shuffle=False)
        all_labels, all_scores, all_weights = [], [], []
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                all_weights.append(batch.weight.cpu().numpy())
...
def create_weighted_histogram(scores: np.ndarray, weights: np.ndarray, name: str,
    ...
    n = len(scores)
    if n > 0:
        arr_x = np.ascontiguousarray(scores, dtype=np.float64)
        arr_w = np.ascontiguousarray(weights, dtype=np.float64)
        hist.FillN(n, arr_x, arr_w, 1)
