...
class ResultPersistence:
    ...
    def save_predictions_to_root(self, model: torch.nn.Module, data_pipeline: DataPipeline,
        ...
        # Channel and run branches (inferred from node features and era)
        # channel_id: 0 = Run1E2Mu (1e+2mu), 1 = Run3Mu (3mu)
        # run_id: 0 = Run2 (2016-2018), 1 = Run3 (2022-2023)
        channel_id_arr = array("i", [0])
        run_id_arr = array("i", [0])
    ...
    def _save_split_predictions(self, model: torch.nn.Module, data_loader,
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(data_loader):
                ...
                # Keep x and batch indices on CPU for channel inference
                x_cpu = batch.x.cpu()
                ...
                for i, (label, scores) in enumerate(zip(batch.y, out)):
                    ...
                    # Infer channel and run from node features and era
