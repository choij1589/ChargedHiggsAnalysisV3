#!/usr/bin/env python
"""
Shared Dataset Manager for multi-process GA optimization in ParticleNetMD.
...
- Handles mass tensors (mass1, mass2) for DisCo loss
- Uses DynamicDatasetLoader for multi-class loading with subsampling
- Shares all tensors including mass variables across worker processes
"""
...
class SharedDatasetManager:
    ...
    def prepare_shared_datasets(self,
                                ...
                                max_valid_events_per_fold: Optional[int] = None,
        ...
        Args:
            ...
            test_folds: Optional list of test fold indices. When provided the return
                        value is (train_batch, valid_batch, test_batch); otherwise
                        the original 2-tuple (train_batch, valid_batch) is returned
                        for backward compatibility.
            pilot: Whether to use pilot datasets
            max_events_per_fold: Maximum events per fold per class (train)
            max_valid_events_per_fold: Maximum events per fold per class for validation.
                                       Defaults to max_events_per_fold if not set.
            balance_weights: Whether to balance class weights
            random_state: Random seed for reproducibility

        Returns:
            (train_batch, valid_batch) when test_folds is None, or
            (train_batch, valid_batch, test_batch) when test_folds is provided.
            All Batch objects have tensors in shared memory.
        """

        # Create cache key
        valid_key = '_'.join(map(str, valid_folds))
        test_key = '_'.join(map(str, test_folds)) if test_folds else 'none'
        cache_key = f"{signal_sample}_{channel}_{'_'.join(map(str, train_folds))}_{valid_key}_{test_key}_{pilot}"
        ...
        # Load validation data (may use a separate cap from train)
        valid_max = max_valid_events_per_fold if max_valid_events_per_fold is not None else max_events_per_fold
        ...
        if test_folds is not None:
            test_data_list = loader.load_multiclass_with_subsampling(
                signal_sample=signal_sample,
                background_groups=background_groups,
                channel=channel,
            ...
            test_shared = self._move_to_shared_memory(test_data_list, "test")
            self.logger.info(f"Test dataset: {test_shared.num_graphs} events")
            self.logger.info("=" * 60)

            # Cache for reuse (including test data)
            self.shared_cache[cache_key] = (train_shared, valid_shared, test_shared)
    ...
    def _move_to_shared_memory(self, data_list: List[Data], dataset_type: str) -> Batch:
        ...
        for key in full_batch.keys():
            ...
            if torch.is_tensor(value):
                ...
                tensor_info.append((key, value.shape, tensor_bytes))
        ...
        # Verify mass tensors are shared (critical for ParticleNetMD DisCo loss)
        if hasattr(full_batch, 'mass1') and hasattr(full_batch, 'mass2'):
            self.logger.info(f"  mass1 shape: {full_batch.mass1.shape}")
            self.logger.info(f"  mass2 shape: {full_batch.mass2.shape}")
        else:
            self.logger.warning("Mass tensors (mass1, mass2) not found in batch!")
            self.logger.warning("DisCo loss requires mass tensors - ensure dataset contains them")
    ...
    def get_memory_usage(self) -> Dict[str, float]:
        ...
        for key, batches in self.shared_cache.items():
            dataset_bytes = 0
