...
class ROCCurveCalculator:
    ...
    def plot_multiclass_rocs(self, y_true_train: np.ndarray, y_scores_train: np.ndarray,
        ...
        canvas = CMS.cmsCanvas(
            ...
            0, 1,  # y-axis: background efficiency from 0 to 1
            "Signal Efficiency",
        ...
        # Create legend
        legend = CMS.cmsLeg(0.20, 0.60, 0.68, 0.75, textSize=0.030, columns=1)

        # Draw diagonal reference line (from (0,0) to (1,1))
        ...
        for bg_class in [1, 2, 3]:  # Background classes
            ...
            # Create TGraph: (signal_eff, bg_eff) = (tpr, fpr)
            n_points = len(fpr_train)
            roc_graph_train = ROOT.TGraph(n_points)
            for i in range(n_points):
                roc_graph_train.SetPoint(i, tpr_train[i], fpr_train[i])

            # Train: lighter color, dashed line
            CMS.cmsObjectDraw(roc_graph_train, "L", LineColor=bg_color_light, LineWidth=2, LineStyle=ROOT.kDashed)
            ...
            for i in range(n_points):
                roc_graph_test.SetPoint(i, tpr_test[i], fpr_test[i])
            ...
            # Merged legend: "Signal vs X: AUC = test (train)"
            CMS.addToLegend(legend, (roc_graph_test, f"Signal vs {bg_name}: AUC = {auc_test:.4f} ({auc_train:.4f})", "L"))
