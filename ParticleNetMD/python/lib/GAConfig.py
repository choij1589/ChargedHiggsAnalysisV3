#!/usr/bin/env python
"""Configuration loader for Genetic Algorithm optimization in ParticleNetMD."""
...
class GAConfigLoader:
    """Loads and processes GA configuration from JSON file for ParticleNetMD."""
    ...
    def get_disco_parameters(self) -> Dict[str, Any]:
        """Get DisCo loss parameters (ParticleNetMD-specific).

        Returns:
            Dict with disco_lambda (fixed, not optimized by GA)
        """
        return self._get_config_section('disco_parameters')

    def get_background_config(self) -> Dict[str, Any]:
        """Get full background configuration including mode and groups."""
    ...
    def get_background_groups(self) -> Dict[str, List[str]]:
        ...
        return bg_config.get('background_groups', {})
    ...
    def generate_hyperparameter_spaces(self) -> List[List[Any]]:
        ...
        Returns:
            List of 5 parameter spaces: [nNodes, optimizers, initLR, weight_decay, schedulers]
        """
        ...
        # Log-uniform sampled parameters (seeded for reproducibility across runs)
        for offset, param_name in enumerate(['initLR', 'weight_decay']):
            ...
            values = [
                ...
                for value in loguniform.rvs(
                    ...
                    random_state=np.random.RandomState(42 + offset)
                )
            ]
    ...
    def get_overfitting_penalty_weight(self) -> float:
        """Get overfitting penalty weight (lambda)."""
        return self.config['ga_parameters'].get('overfitting_penalty_weight', 0.0)
    ...
    def _validate_config(self) -> None:
        ...
        self._validate_disco_parameters()
    ...
    def _validate_ga_parameters(self) -> None:
        ...
        # Validate overfitting penalty weight if present
        penalty_weight = ga_params.get('overfitting_penalty_weight', 0.0)
        self._validate_type(penalty_weight, (int, float), 'overfitting_penalty_weight')
        if penalty_weight < 0:
            raise ValueError(f"overfitting_penalty_weight must be non-negative, got {penalty_weight}")
    ...
    def _validate_training_parameters(self) -> None:
        ...
        # Validate loss_type for ParticleNetMD
        loss_type = train_params.get('loss_type', 'disco')
        valid_loss_types = ['disco', 'weighted_ce', 'focal']
    ...
    def _validate_overfitting_config(self) -> None:
        ...
        if 'enabled' in overfitting_config:
            ...
            if enabled:
                ...
                # Validate num_ks_tests for ParticleNetMD (16 = 4 classes x 4 scores)
                num_ks_tests = overfitting_config.get('num_ks_tests', 16)
                self._validate_positive_int(num_ks_tests, 'overfitting_detection.num_ks_tests')

    def _validate_disco_parameters(self) -> None:
        """Validate DisCo loss parameters (ParticleNetMD-specific)."""
        disco_params = self.config.get('disco_parameters', {})

        disco_lambda = disco_params.get('disco_lambda')
        if disco_lambda is not None:
            self._validate_type(disco_lambda, (int, float), 'disco_lambda')
            if disco_lambda < 0:
                raise ValueError(f"disco_lambda must be non-negative, got {disco_lambda}")
    ...
    def summary(self) -> str:
        ...
        lines = ["=" * 60, "GA CONFIGURATION SUMMARY (ParticleNetMD)", "=" * 60]
        ...
        lines.extend([
            ...
            f"Overfitting penalty weight: {ga_params.get('overfitting_penalty_weight', 0.0)}"
        ])

        # DisCo parameters (ParticleNetMD-specific)
        disco_params = self.get_disco_parameters()
        lines.extend([
            "\nDisCo Parameters (Fixed):",
            f"  disco_lambda: {disco_params.get('disco_lambda', 0.05)}"
        ])
        ...
        for group_name, samples in self.get_background_groups().items():
            lines.append(f"  {group_name}: {len(samples)} samples")
        ...
        overfitting_config = self.get_overfitting_config()
        lines.extend([
            "\nOverfitting Detection:",
            f"  Enabled: {overfitting_config.get('enabled', False)}",
            f"  P-value threshold: {overfitting_config.get('p_value_threshold', 0.05)}",
            f"  Number of KS tests: {overfitting_config.get('num_ks_tests', 16)}",
            "=" * 60
        ])
