#!/usr/bin/env python3
"""
Comprehensive visualization and overfitting analysis for GA iteration results in ParticleNetMD.

This script evaluates all models from a GA iteration, performs KS tests on all
output scores for each class, and generates comprehensive visualizations including
mass decorrelation analysis specific to ParticleNetMD.

For each model:
- Performs 16 KS tests (4 true classes x 4 output scores)
- Generates score distribution plots
- Creates ROC curves and confusion matrices
- Produces training curves with CE Loss, DisCo Term, and Accuracy
- Creates mass decorrelation plots (score vs mass, mass profile, mass sculpting)
- Saves overfitting diagnostic results

Usage:
    python visualizeGAIteration.py --signal MHc130_MA90 --channel Combined --iteration 0 --device cuda:0 --pilot
"""
...
def evaluate_model(model: torch.nn.Module, loader: DataLoader, device: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    with torch.no_grad():
        for data in loader:
            ...
            # Get mass variables if available
            if hasattr(data, 'mass1'):
                mass1_list.append(data.mass1)
    ...
    if mass1_list:
        mass1 = torch.cat(mass1_list).cpu().numpy()
    else:
        mass1 = np.zeros(len(y_true))

    if mass2_list:
        mass2 = torch.cat(mass2_list).cpu().numpy()
    else:
...
# =============================================================================
# Histogram Utilities for KS Tests
# =============================================================================
...
def plot_training_curves(config_data: Dict, output_path: str, model_idx: int):
    """Plot training curves with 3 panels: CE Loss, DisCo Term, Accuracy (ParticleNetMD-specific)."""
    ...
    canvas = ROOT.TCanvas(f"c_training_model{model_idx}", "Training Curves", 2100, 600)
...
def plot_score_vs_mass_2d(mass1: np.ndarray, y_scores: np.ndarray, y_true: np.ndarray,
                          weights: np.ndarray, output_dir: str, model_idx: int):
    ...
    for class_idx, class_name in enumerate(CLASS_DISPLAY_NAMES):
        class_mask = (y_true == class_idx) & valid_mask

        if np.sum(class_mask) < 10:
            print(f"  Warning: Not enough events for {class_name} in score vs mass 2D plot")
            continue

        h2d = ROOT.TH2F(f"h2d_score_mass_{model_idx}_{class_idx}",
        ...
        canvas = ROOT.TCanvas(f"c_2d_{model_idx}_{class_idx}", "", 800, 700)
        ...
        latex.DrawLatex(0.15, 0.85, f"Model {model_idx} - True: {class_name}")
        ...
        output_file = os.path.join(output_dir, f'model{model_idx}_score_vs_mass_{class_name.lower()}.png')
        canvas.SaveAs(output_file)
        canvas.Close()


def plot_mass_profile_vs_score(mass1: np.ndarray, y_scores: np.ndarray, weights: np.ndarray,
                                output_path: str, model_idx: int):
    ...
    h_profile = ROOT.TProfile(f"h_profile_{model_idx}",
                               "Mean Mass vs Signal Score;Signal Score;<m_{#mu#mu}> [GeV]",
                               20, 0, 1, 0, 200)
    ...
    canvas = ROOT.TCanvas(f"c_profile_{model_idx}", "", 800, 600)
    canvas.SetLeftMargin(0.12)
    canvas.SetBottomMargin(0.12)
    canvas.SetGrid()
    ...
    latex.DrawLatex(0.15, 0.85, f"Model {model_idx}")
...
def load_datasets(config, signal: str, channel: str, pilot: bool, device: str):
    ...
    # Setup dataset root - ParticleNetMD specific path
...
def create_summary_plots(all_results: List[Dict], model_info_df: Dict, output_dir: str):
    ...
    ax.scatter(fitness_scores, min_p_values, c=colors, s=100, alpha=0.6, edgecolors='black')
...
def save_summary_text(all_results: List[Dict], model_info_df: Dict, output_path: str):
    ...
    with open(output_path, 'w') as f:
        f.write("="*80 + "\n")
        f.write("GA ITERATION OVERFITTING ANALYSIS SUMMARY (ParticleNetMD)\n")
        f.write("="*80 + "\n\n")
