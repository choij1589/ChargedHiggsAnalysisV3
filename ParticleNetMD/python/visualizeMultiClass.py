...
This script generates individual plots for multi-class classification models with
mass decorrelation (DisCo) training:
...
Mass Decorrelation Plots (NEW):
- DisCo evolution (CE loss + DisCo term vs epoch)
- Score vs mass 2D histograms (per true class)
- Mass profile vs score (mean mass in score bins)
- Mass sculpting comparison (mass distribution at high vs low score)
...
def find_multiclass_results(signal, channel, fold, pilot=False):
...
def load_training_data(ga_json_file):
    ...
    # Extract training history from epoch_history (column-based arrays)
    epoch_history = data.get('epoch_history', {})

    # Convert column-based to row-based DataFrame
    df = pd.DataFrame(epoch_history)

    # Define class names based on background groups in hyperparameters
    # Default: Signal + 3 background groups
...
def load_multiclass_predictions_from_root(root_file):
    ...
    Uses PyROOT directly (more reliable than uproot in HEP environments).

    Args:
        root_file: Path to the ROOT file containing the Events tree
    ...
    Returns:
        Tuple of (y_true_train, y_scores_train, weights_train, mass1_train, mass2_train,
                  ...
                  class_names, has_bjet_train, has_bjet_test)
    """
    ...
    try:
        ...
        valid_mask = np.zeros(n_entries, dtype=bool)
        test_mask = np.zeros(n_entries, dtype=bool)
        has_bjet = np.zeros(n_entries, dtype=bool)
        ...
        logging.info("Loaded mass1 and mass2 for decorrelation analysis")
        ...
        valid_mass2 = mass2[mass2 > 0]
        if len(valid_mass1) > 0:
            logging.info(f"Mass1 range: {valid_mass1.min():.1f} to {valid_mass1.max():.1f} GeV")
        if len(valid_mass2) > 0:
            logging.info(f"Mass2 range: {valid_mass2.min():.1f} to {valid_mass2.max():.1f} GeV")
...
# =============================================================================
# Mass Decorrelation Visualization Functions (NEW for ParticleNetMD)
# =============================================================================

def plot_disco_evolution(json_data, signal, output_path):
    """
    Plot CE loss and DisCo term evolution during training.

    Shows how the cross-entropy loss and distance correlation regularization
    term evolve separately across epochs.
    """
    epoch_history = json_data.get('epoch_history', {})

    # Check if decomposed losses are available
    if 'train_ce_loss' not in epoch_history:
        logging.info("No decomposed loss data available (not DisCo training) - skipping disco_evolution plot")
        return

    epochs = epoch_history['epoch']
    train_ce = epoch_history['train_ce_loss']
    train_disco = epoch_history['train_disco_term']
    valid_ce = epoch_history['valid_ce_loss']
    valid_disco = epoch_history['valid_disco_term']

    disco_lambda = json_data.get('hyperparameters', {}).get('disco_lambda', 0.1)

    # Create figure with 2 subplots
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))

    # Left: CE loss
    ax1.plot(epochs, train_ce, 'b-', label='Train CE', linewidth=2)
    ax1.plot(epochs, valid_ce, 'r--', label='Valid CE', linewidth=2)
    ax1.set_xlabel('Epoch')
    ax1.set_ylabel('Cross-Entropy Loss')
    ax1.legend()
    ax1.set_title('CE Loss Evolution')
    ax1.grid(True, alpha=0.3)

    # Right: DisCo term
    ax2.plot(epochs, train_disco, 'b-', label='Train DisCo', linewidth=2)
    ax2.plot(epochs, valid_disco, 'r--', label='Valid DisCo', linewidth=2)
    ax2.set_xlabel('Epoch')
    ax2.set_ylabel('DisCo Term (Distance Correlation)')
    ax2.legend()
    ax2.set_title(f'DisCo Term Evolution ($\\lambda$={disco_lambda})')
    ax2.grid(True, alpha=0.3)

    plt.suptitle(f'Loss Decomposition: {signal}', fontsize=14)
    plt.tight_layout()

    output_file = os.path.join(output_path, 'disco_evolution.png')
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    plt.close()

    logging.info(f"DisCo evolution plot saved to: {output_file}")

def plot_score_vs_mass_2d(mass1, y_scores, y_true, weights, class_names, signal, output_path):
    """
    Plot 2D histogram of signal score vs di-muon mass for each true class.

    This shows how the classifier score correlates with mass for different
    event types. A well-decorrelated classifier should show uniform score
    distribution across mass.
    """
    ROOT.gROOT.SetBatch(True)
    CMS.setCMSStyle()

    signal_scores = y_scores[:, 0]  # Signal class probability
...
def plot_mass_profile_vs_score(mass1, y_scores, weights, signal, output_path):
    """
    Plot mean mass as function of signal score (profile plot).

    A flat profile indicates good decorrelation - the classifier score
    doesn't systematically shift with mass.
    """
    ROOT.gROOT.SetBatch(True)
    CMS.setCMSStyle()
    ...
    # Draw reference line (expected flat if decorrelated)
...
def plot_mass_sculpting(mass1, y_scores, weights, signal, output_path):
    """
    Compare mass distribution at high vs low signal score.

    If the classifier is well-decorrelated, the mass distributions should
    be similar regardless of score cut. Different shapes indicate mass sculpting.
    """
    ROOT.gROOT.SetBatch(True)
    CMS.setCMSStyle()
...
# =============================================================================
# End Mass Decorrelation Functions
# =============================================================================
...
def main():
    ...
    try:
        ...
        ga_json_file, root_file, result_dir = find_multiclass_results(
            args.signal, args.channel, args.fold, args.pilot)

        data, df, class_names = load_training_data(ga_json_file)

        # Generate training curves
        logging.info("Generating training curves...")
        plot_training_curves(df, args.signal, args.output)

        # Generate DisCo evolution plot (NEW)
        logging.info("Generating DisCo evolution plot...")
        plot_disco_evolution(data, args.signal, args.output)
        ...
        (y_true_train, y_scores_train, weights_train, mass1_train, mass2_train,
         y_true_test, y_scores_test, weights_test, mass1_test, mass2_test,
         class_names, has_bjet_train, has_bjet_test) = load_multiclass_predictions_from_root(root_file)
        ...
        # =====================================================================
        # Mass Decorrelation Plots (NEW for ParticleNetMD)
        # =====================================================================
