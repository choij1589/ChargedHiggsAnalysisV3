...
# Always read samples from Baseline directory
BASEDIR = f"{WORKDIR}/SignalRegionStudy/samples/{args.era}/{args.channel}/{args.masspoint}/Baseline"
OUTDIR = f"{WORKDIR}/SignalRegionStudy/templates/{args.era}/{args.channel}/{args.masspoint}/Shape/ParticleNet"

logging.info(f"Input directory: {BASEDIR}")
logging.info(f"Output directory: {OUTDIR}")

# Load mass window from signal fit results
fit_json_path = f"{OUTDIR}/signal_fit.json"
if not os.path.exists(fit_json_path):
    raise FileNotFoundError(
        f"Signal fit results not found: {fit_json_path}\n"
        f"Please run makeBinnedTemplates.py first:\n"
        f"  makeBinnedTemplates.py --era {args.era} --channel {args.channel} --masspoint {args.masspoint} --method ParticleNet"
    )

with open(fit_json_path, 'r') as f:
    fit_params = json.load(f)

MASS_MIN = fit_params["mass_min"]
MASS_MAX = fit_params["mass_max"]

logging.info(f"Mass window from fit: [{MASS_MIN:.2f}, {MASS_MAX:.2f}] GeV")
...
if __name__ == "__main__":
    ...
    # Create histograms for backgrounds (for stacking)
    bkg_hists = {}

    # Nonprompt
    hist_nonprompt = createHistogram("nonprompt", args.masspoint, "Nonprompt", ROOT.kAzure + 2)
    if hist_nonprompt.Integral() > 0:
        bkg_hists["nonprompt"] = hist_nonprompt

    # Conversion
    hist_conversion = createHistogram("conversion", args.masspoint, "Conversion", ROOT.kOrange + 1)
    if hist_conversion.Integral() > 0:
        bkg_hists["conversion"] = hist_conversion

    # Diboson
    hist_diboson = createHistogram("diboson", args.masspoint, "Diboson", ROOT.kGreen + 2)
    if hist_diboson.Integral() > 0:
        bkg_hists["diboson"] = hist_diboson

    # ttX
    hist_ttX = createHistogram("ttX", args.masspoint, "ttX", ROOT.kMagenta + 2)
    if hist_ttX.Integral() > 0:
        bkg_hists["ttX"] = hist_ttX

    # Others
    hist_others = createHistogram("others", args.masspoint, "Others", ROOT.kGray + 1)
    ...
    # For blind analysis, we don't include signal in data_obs
