...
if args.isQCD:
    ...
    file_path = f"{WORKDIR}/MeasFakeRateV3/results/{args.era}/ROOT/{args.measure}/fakerate_TTJJ.root"
elif args.isMC:
    file_path = f"{WORKDIR}/MeasFakeRateV3/results/{args.era}/ROOT/{args.measure}/fakerate_MC.root"
else:
...
if args.isMC:
    ...
    base_output = f"{WORKDIR}/MeasFakeRateV3/plots/{args.era}/{args.measure}"
    ...
    h.SetDirectory(0)
    f.Close()
    output_path = f"{WORKDIR}/MeasFakeRateV3/plots/{args.era}/{args.measure}/fakerate_qcd.png"
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    ...
    h.SetDirectory(0)
    f.Close()
    output_path = f"{WORKDIR}/MeasFakeRateV3/plots/{args.era}/{args.measure}/fakerate_TTJJ.png"
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    plot_fakerate(h, output_path)
else:
