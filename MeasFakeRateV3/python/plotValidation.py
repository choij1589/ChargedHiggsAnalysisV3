...
for ptcorr, abseta in product(ptcorr_bins[:-1], abseta_bins[:-1]):
    ...
    for DATAPERIOD in DATAPERIODs:
        file_path = f"{WORKDIR}/SKNanoOutput/{analyzer}/{args.hlt}_RunSyst/{args.era}/{DATAPERIOD}.root"
    ...
    for sample in MCList:
        file_path = f"{WORKDIR}/SKNanoOutput/{analyzer}/{args.hlt}_RunSyst/{args.era}/{sample}.root"
        assert os.path.exists(file_path), f"{file_path} does not exist"
        f = ROOT.TFile.Open(file_path)
        try:
            h = f.Get(f"{prefix}/{args.region}/{args.wp}/{args.selection}/MT")
    ...
    json_path = f"{WORKDIR}/MeasFakeRateV3/results/{args.era}/JSON/{MEASURE}/prompt_scale.json"
    ...
    output_path = f"{WORKDIR}/MeasFakeRateV3/plots/{args.era}/{MEASURE}/{args.hlt}/{args.region}/{args.selection}/{prefix}_{args.wp}_MT.png"
