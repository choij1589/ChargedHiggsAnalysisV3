...
#
# Usage:
#   ./runImpacts.sh --era 2018 --channel SR1E2Mu --masspoint MHc130_MA90 --method Baseline --binning extended
#   ./runImpacts.sh --era 2018 --channel SR1E2Mu --masspoint MHc130_MA90 --method Baseline --binning extended --condor
#
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --r-range    Override r range as 'rMin,rMax' [default: wider of auto asymptotic range and -5,5]"
...
if [[ -n "$R_RANGE_OVERRIDE" ]]; then
    ...
    AUTO_ABS_RMAX=$(python3 -c "print(abs(float('${AUTO_RMAX}')))")
    USE_AUTO_RANGE=$(python3 -c "print('1' if float('${AUTO_ABS_RMAX}') > 5.0 else '0')")
    if [[ "$USE_AUTO_RANGE" == "1" ]]; then
        R_RANGE="r=${AUTO_RMIN},${AUTO_RMAX}"
        echo "  Auto r-range from asymptotic is wider than -5,5: ${R_RANGE}"
    else
        R_RANGE="r=-5,5"
        echo "  Auto r-range from asymptotic is narrower (${AUTO_RMIN},${AUTO_RMAX}); using ${R_RANGE}"
    fi
else
...
elif [[ "$AUTO_EXPECT_SIGNAL" == true ]]; then
...
if [[ "$DRY_RUN" == true && ! -d "$TEMPLATE_DIR" ]]; then
    echo "[DRY-RUN] Template directory does not exist yet: $TEMPLATE_DIR"
    echo "[DRY-RUN] Would create workspace and submit impact workflow under:"
    echo "[DRY-RUN]   ${OUTPUT_DIR}"
    exit 0
fi
...
# Build fit options: use Asimov (expected) for blinded SR, real data for
# unblinded SR and CR validation fits.
if [[ "$METHOD" == "CR" ]]; then
    ASIMOV_OPTIONS=""
    BLIND_OPT=""
elif [[ "$UNBLIND" == true || "$PARTIAL_UNBLIND" == true ]]; then
