...
#
# This script supports Condor-only execution for SignalRegionStudyV3.
...
generate_hybridnew_plots() {
    ...
    python3 ${WORKDIR}/SignalRegionStudyV3/python/plotHybridNewGrid.py \
        "${INPUT_DIR}" \
...
for r in sorted(completed.keys()):
    if completed[r] >= 2:
        ...
        script_path = os.path.join(workdir, "SignalRegionStudyV3/python/plotTestStatDist.py")
