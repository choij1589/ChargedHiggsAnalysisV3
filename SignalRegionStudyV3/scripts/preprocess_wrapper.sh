...
ln -sf "$PNFS_BASE/SignalRegionStudyV3/samples" "$LOCAL_WORKDIR/SignalRegionStudyV3/samples_source" 2>/dev/null || true

# Run preprocessing with local WORKDIR
export WORKDIR="$LOCAL_WORKDIR"
python3 "$REPO_DIR/SignalRegionStudyV3/python/preprocess.py" \
    --era "$ERA" --channel "$CHANNEL" --masspoint "$MASSPOINT" $EXTRA_ARGS

# Copy output to pnfs via xrootd (faster than NFS for writes)
LOCAL_OUTPUT="$LOCAL_WORKDIR/SignalRegionStudyV3/samples/$ERA/$CHANNEL/$MASSPOINT"
SE_OUTPUT="$SE_BASE/SignalRegionStudyV3/samples/$ERA/$CHANNEL/$MASSPOINT"
PNFS_OUTPUT="$PNFS_BASE/SignalRegionStudyV3/samples/$ERA/$CHANNEL/$MASSPOINT"
