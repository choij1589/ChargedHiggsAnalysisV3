...
PNFS_BASE="/pnfs/knu.ac.kr/data/cms/store/user/choij/SignalRegionStudyV3/samples"
XROOTD_BASE="root://cluster142.knu.ac.kr//store/user/choij/SignalRegionStudyV3/samples"
...
run_template_local() {
    ...
    mkdir -p "$local_workdir/SignalRegionStudyV3"

    # Create symlinks for code (read-only, from scratch)
    ln -sf "$SCRATCH_WORKDIR/SignalRegionStudyV3/python" "$local_workdir/SignalRegionStudyV3/python"
    ln -sf "$SCRATCH_WORKDIR/SignalRegionStudyV3/scripts" "$local_workdir/SignalRegionStudyV3/scripts"
    ln -sf "$SCRATCH_WORKDIR/SignalRegionStudyV3/configs" "$local_workdir/SignalRegionStudyV3/configs"
    ln -sf "$SCRATCH_WORKDIR/Common" "$local_workdir/Common"

    # Create local samples and templates directories
    mkdir -p "$local_workdir/SignalRegionStudyV3/samples"
    mkdir -p "$local_workdir/SignalRegionStudyV3/templates"
    ...
    for era_to_copy in "${eras_to_copy[@]}"; do
        for channel_to_copy in "${channels_to_copy[@]}"; do
            copy_inputs_to_local "$era_to_copy" "$channel_to_copy" "$MASSPOINT" \
                "$local_workdir/SignalRegionStudyV3/samples"
        done
    done

    # Set WORKDIR to local scratch for processing
    export WORKDIR="$local_workdir"
    cd "$local_workdir/SignalRegionStudyV3"
    ...
    local local_output="$local_workdir/SignalRegionStudyV3/templates/$ERA/$CHANNEL/$MASSPOINT/$METHOD/$BINNING_SUFFIX"
    local scratch_output="$SCRATCH_WORKDIR/SignalRegionStudyV3/templates/$ERA/$CHANNEL/$MASSPOINT/$METHOD/$BINNING_SUFFIX"
...
run_on_scratch() {
    ...
    cd "$SCRATCH_WORKDIR/SignalRegionStudyV3"
    export PATH="${PWD}/python:${PATH}"
