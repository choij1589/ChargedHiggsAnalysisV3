#!/bin/bash
#
# runSignalStrength.sh - Run observed MultiDimFit signal strength from workspace.root.
#
# Usage:
#   ./scripts/runSignalStrength.sh --era All --channel Combined \
...
OUTPUT_DIR="combine_output/signal_strength"
NAME=".${MASSPOINT}.${METHOD}.${BINNING_SUFFIX}.SignalStrength"

echo "Running MultiDimFit signal strength for ${MASSPOINT} (${ERA}/${CHANNEL}/${METHOD}/${BINNING_SUFFIX})"
...
COMBINE_CMD="combine -M MultiDimFit ${WORKSPACE} \
    --algo singles \
    --rMin=${RMIN} --rMax=${RMAX} \
    --robustFit 1 \
    -n ${NAME} \
...
if [[ "$DRY_RUN" == false ]]; then
    mv -f higgsCombine.*.MultiDimFit.*.root "$OUTPUT_DIR/" 2>/dev/null || true
    mv -f multidimfit*.root "$OUTPUT_DIR/" 2>/dev/null || true
