...
SignalRegionStudyV3 uses Run-period component templates as its default Combine
model. The legacy subera-category likelihood is not preserved in V3; use
SignalRegionStudyV2 for comparisons to that construction.
...
The likelihood is intentionally not identical to the old subera-category model.
...
1. Fit the combined category signal distribution with the existing DCB model.
...
6. If 5 core bins still fails, keep 5 core bins plus two sideband bins and use
   the existing floor/zero low-stat handling.
...
The output directory remains the normal V3 template path:

```text
templates/{Run2,Run3,All}/{SR1E2Mu,SR3Mu,Combined}/{masspoint}/{method}/{binning}/
```

The TTZ2E1Mu control-region GoF path uses the same construction as a validation
test of the model. It writes:

```text
templates/{Run2,Run3,All}/TTZ2E1Mu/MHc130_MA90/CR/ZWin_adaptive/
```

with categories such as `TTZ2E1Mu_Run2` and subera components such as
`ttZ_2018`, `nonprompt_2017`, and dummy `signal_2018` placeholders. The CR
uses real data, so validation skips blinded `data_obs == total background`
closure for `method=CR`.
...
Each merged category receives its own `autoMCStats 5` line. The expected
behavior is that Combine evaluates the threshold on the merged category/bin
total; this must be checked from `text2workspace.py` logs and workspace nuisance
...
- `autoMCStats` nuisance count/type is compared to V2/reference workspaces.
- Saturated GoF is compared for representative low-`mA` and nominal mass points.
