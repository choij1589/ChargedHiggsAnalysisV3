...
(see `calculate_adaptive_bins` in `template_utils.py`). In V3 the scan is done
per merged Run-period category using the summed category background over all
subera component processes.

For the standard V3 `extended` binning, loop:
`n_core in [15, 14, 13, 12, 11, 10, 9, 8, 7, 6, 5]`.

For `uniform` binning, loop:
`n_core in [15, 13, 11, 9, 7, 5]`.

For the TTZ2E1Mu control-region `ZWin_adaptive` templates, the full
`[81.2, 101.2] GeV` Z window is treated as the core region. The Run-period
component builder scans the full-window uniform bin count as:
`n_bins in [15, 14, 13, 12, 11, 10, 9, 8, 7, 6, 5]`.
...
If the `for` loop exhausts without a `break`, the coarsest candidate for that
binning mode is kept and a fallback is flagged (see B3). For `extended`, this
means 1 core bin; for `uniform`, this means 5 core bins.
...
at its coarsest candidate. Recorded in `binning.json` as
`floor_applied: true`.
