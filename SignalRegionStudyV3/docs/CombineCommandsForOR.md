...
For grouped impact breakdowns, define nuisance groups in a temporary datacard and use cumulative `MultiDimFit` scans with `--freezeNuisanceGroups`. In V3 this workflow is wrapped by `scripts/runImpactBreakdown.sh`; it does not modify the production datacard. The compact breakdown uses `signal_theory`, `prompt_norm`, `nonprompt_norm`, `experimental`, and a residual `stat` component that includes both generated autoMCStats and the remaining statistical uncertainty. The last cumulative group scan uses `--X-rtd MINIMIZER_no_analytic` so that this endpoint is written as a full grid rather than only the best-fit point.
