...
Usage:
    checkTemplates.py --era 2018 --channel SR1E2Mu --masspoint MHc130_MA90 --method Baseline --binning extended
"""
import os
import sys
import json
...
parser.add_argument("--era", required=True, type=str, help="Data-taking period or run period (Run2, Run3, All, 2018, etc.)")
...
for era in era_list:
    for channel in channel_list:
        template_dirs.append({
            "era": era,
            "channel": channel,
            "path": f"{WORKDIR}/SignalRegionStudyV3/templates/{era}/{channel}/{args.masspoint}/{args.method}/{binning_suffix}"
        })

# Prefer the requested V3 output directory. Run-period component templates are
# built directly under args.era/args.channel, not under subera template dirs.
REQUESTED_TEMPLATE_DIR = f"{WORKDIR}/SignalRegionStudyV3/templates/{args.era}/{args.channel}/{args.masspoint}/{args.method}/{binning_suffix}"
if os.path.exists(f"{REQUESTED_TEMPLATE_DIR}/categories.json"):
    template_dirs = [{
        "era": args.era,
        "channel": args.channel,
        "path": REQUESTED_TEMPLATE_DIR,
    }]
    TEMPLATE_DIR = REQUESTED_TEMPLATE_DIR
else:
    # Legacy fallback for non-run-period artifacts.
    TEMPLATE_DIR = template_dirs[0]["path"]

# Output directory for validation (use combined era/channel name)
VALIDATION_DIR = f"{WORKDIR}/SignalRegionStudyV3/templates/{args.era}/{args.channel}/{args.masspoint}/{args.method}/{binning_suffix}/validation"
...
def load_systematics_config():
    ...
    if len(era_list) == 1 and len(channel_list) == 1:
        ...
        config_path = f"{WORKDIR}/SignalRegionStudyV3/configs/systematics.{era_list[0]}.json"
...
if __name__ == "__main__":
    ...
    if os.path.exists(categories_path):
        logging.info("Detected run_period_components templates; using validateRunPeriodTemplates.py")
        cmd = [
            sys.executable, "python/validateRunPeriodTemplates.py",
            "--era", args.era,
            "--channel", args.channel,
