...
Usage:
    combineDatacardsCnC.py --mode channel --era 2018 --masspoint MHc130_MA90 --method Baseline --binning extended
    combineDatacardsCnC.py --mode era --channel Combined --masspoint MHc130_MA90 --method Baseline --binning extended
"""
import os
import sys
import subprocess
...
# Era and channel definitions for the CnC-only datacard combiner.
...
def combine_channels(era, masspoint, method, binning, workdir, nsigma_tag="3sigma"):
    ...
    output_dir = Path(f"{workdir}/SignalRegionStudyV3/templates/{era}/Combined/{masspoint}/{method}/{binning}")
    output_dir.mkdir(parents=True, exist_ok=True)

    datacards = {}
    for channel in CHANNELS:
        card_path = f"{workdir}/SignalRegionStudyV3/templates/{era}/{channel}/{masspoint}/{method}/{binning}/datacard_cnc_{nsigma_tag}.txt"
...
def combine_eras(eras, channel, masspoint, method, binning, output_era, workdir, nsigma_tag="3sigma"):
    ...
    output_dir = Path(f"{workdir}/SignalRegionStudyV3/templates/{output_era}/{channel}/{masspoint}/{method}/{binning}")
    output_dir.mkdir(parents=True, exist_ok=True)

    datacards = {}
    for era in eras:
        card_path = Path(f"{workdir}/SignalRegionStudyV3/templates/{era}/{channel}/{masspoint}/{method}/{binning}/datacard_cnc_{nsigma_tag}.txt")
        if card_path.exists():
