...
from plotter import ComparisonCanvas, KinematicCanvas, PALETTE_LONG, get_CoM_energy, _LUMI_CONFIG  # noqa: E402
...
def process_names_for_variation(payload, include_signal=False):
    return {
        meta["name"]
        for meta in payload["processes"]
...
def nuisance_has_category_targets(category, payload, datacard_nuisances, syst_name, include_signal=False):
    targets = process_names_for_variation(payload, include_signal=include_signal)
    for row in datacard_nuisances["rows"]:
...
def variation_target_kind(category, payload, datacard_nuisances, syst_name):
    if nuisance_has_category_targets(category, payload, datacard_nuisances, syst_name, include_signal=False):
        return "background"
    if nuisance_has_category_targets(category, payload, datacard_nuisances, syst_name, include_signal=True):
        return "signal"
    return None


def varied_total_for_syst(directory, category, payload, datacard_nuisances, syst_name, direction, target_kind):
    row = next(
    ...
    process_names = process_names_for_variation(payload, include_signal=(target_kind == "signal"))
    total = None
    for cat_col, proc, value in zip(datacard_nuisances["bins"], datacard_nuisances["processes"], row["values"]):
        if cat_col != category or proc not in process_names:
            continue
        hist = shifted_hist_for_nuisance(directory, proc, syst_name, row["type"], value, direction)
        if not hist or not hist.InheritsFrom("TH1"):
            continue
        if total is None:
            total = clone_hist(hist, f"{category}_{syst_name}_{direction}")
        else:
            total.Add(hist)
    return total
...
def systematic_scope(syst_name):
    """Return exact era/subera group for decorrelated nuisances, or 'run'."""
...
def systematic_output_scope(syst_name, payload, args):
    ...
    if args.era == "All" and scope == "run":
        return payload.get("period") or payload.get("run_period") or "run"
    return scope
...
def systematic_active_for_category(syst_name, category, payload, datacard_nuisances):
    ...
    if scope in ERA_DECORRELATED_TOKENS and scope not in payload.get("suberas", []):
        return False
    return variation_target_kind(category, payload, datacard_nuisances, syst_name) is not None
...
def make_systematic_plots(f, categories, shape_rows, output_dir, args, datacard_nuisances):
    plots = {"run": []}
    for token in ERA_DECORRELATED_TOKENS:
        plots[token] = []
    plots["uncorrelated"] = []
    ...
    for cat, payload in categories.items():
        ...
        for syst_name in syst_names:
            if not systematic_active_for_category(syst_name, cat, payload, datacard_nuisances):
                continue
            scope = systematic_output_scope(syst_name, payload, args)
            target_kind = variation_target_kind(cat, payload, datacard_nuisances, syst_name)
            if target_kind is None:
                continue
            nominal = varied_total_for_syst(
                directory, cat, payload, datacard_nuisances, syst_name, "Nominal", target_kind
            )
            up = varied_total_for_syst(
                directory, cat, payload, datacard_nuisances, syst_name, "Up", target_kind
            )
            down = varied_total_for_syst(
                directory, cat, payload, datacard_nuisances, syst_name, "Down", target_kind
            )
            if not nominal or not up or not down or nominal.Integral() <= 0:
                continue

            hists = OrderedDict([
                (f"Nominal ({nominal.Integral():.2f})", nominal),
                (f"Up ({up.Integral():.2f})", up),
                (f"Down ({down.Integral():.2f})", down),
            ])
            plot_era = plot_era_for_category(args, payload)
            plot_channel = payload.get("channel", args.channel)
            config = {
            ...
            plotter = ValidationKinematicCanvas(hists, config)
            plotter.palette = config["colors"]
            plotter.drawPad()
            panel_label = syst_name if target_kind == "background" else f"{syst_name} (signal)"
            CMS.drawText(panel_label, posX=0.20, posY=0.58, font=42, align=0, size=0.026)
            plotter.leg.Draw()
            plotter.canv.RedrawAxis()
            scoped_output_dir = os.path.join(output_dir, scope)
            ...
            output_base = os.path.join(scoped_output_dir, output_name)
            save_canvas(plotter.canv, output_base)
            plots.setdefault(scope, []).append(f"{output_base}.png")
    return plots
...
def main():
    ...
    if not args.skip_plots:
        ...
        plot_summary["systematics"] = make_systematic_plots(
            f, categories, shape_rows, syst_plot_dir, args, datacard_nuisances
        )
