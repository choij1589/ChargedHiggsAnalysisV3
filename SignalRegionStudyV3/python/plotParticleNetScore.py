...
BASEDIR = f"{WORKDIR}/SignalRegionStudyV3/samples/{reference_era}/{reference_channel}/{args.masspoint}"
# Output to ParticleNet template directory (with unblind suffix if applicable)
OUTDIR = f"{WORKDIR}/SignalRegionStudyV3/templates/{args.era}/{args.channel}/{args.masspoint}/ParticleNet/{binning_suffix}"
...
histkeys_path = f"{WORKDIR}/SignalRegionStudyV3/configs/histkeys.json"
...
def run_period_for_era(era):
    """Return Run2/Run3 for a concrete data-taking era."""
    if era in ERAS_RUN2:
        return "Run2"
    if era in ERAS_RUN3:
        return "Run3"
...
def template_dir(era, channel, suffix):
    return (
        f"{WORKDIR}/SignalRegionStudyV3/templates/{era}/{channel}/"
...
def category_payload(payload, era, channel):
    """Return the category-specific payload for run-period-component metadata."""
    categories = payload.get("categories")
    if not isinstance(categories, dict):
        return payload

    preferred = [
        category_key_for(era, channel),
        f"{channel}_{args.era}",
        f"{reference_channel}_{run_period_for_era(reference_era) or reference_era}",
    ]
    ...
    if categories:
        ...
        logging.warning("Falling back to first category in metadata: %s", first_key)
        return categories[first_key]
    return payload
...
MASS_MIN, MASS_MAX = mass_window_for(reference_era, reference_channel)
...
def load_background_weights(era, channel):
    ...
    if not os.path.exists(weights_json_path):
        ...
        logging.warning("Using unweighted ParticleNet score (equal priors)")
        return None
    ...
    key = category_key_for(era, channel)
    if key in categories:
        return categories[key].get("weights")
    if categories:
        first_key = next(iter(categories))
        logging.warning("Background weights missing for %s; using %s", key, first_key)
        return categories[first_key].get("weights")
    return None
...
if is_combined_era or is_combined_channel:
    ...
    syst_config_path = f"{WORKDIR}/SignalRegionStudyV3/configs/systematics.{args.era}.json"
...
def load_syst_categories_for(era, channel):
    """Load systematics for an on-demand single-era/channel score histogram."""
    syst_config_path = f"{WORKDIR}/SignalRegionStudyV3/configs/systematics.{era}.json"
    if not os.path.exists(syst_config_path):
        logging.warning(f"Systematics config not found: {syst_config_path}")
        return {'preprocessed_shape': [], 'valued_shape': [], 'multi_variation': [], 'valued_lnN': []}
    with open(syst_config_path) as f:
        syst_config_full = json.load(f)
    return categorize_systematics(syst_config_full.get(channel, {}))
...
def create_histograms(era, channel, masspoint, sample_channel, syst_categories,
    ...
    """
    sample_dir = f"{WORKDIR}/SignalRegionStudyV3/samples/{era}/{sample_channel}/{masspoint}"
        ...
        if not os.path.exists(hist_file):
            ...
            if cache_key not in generated_cache:
                ...
                if not os.path.exists(sample_dir):
                    ...
                    bg_weights = load_background_weights(era, input_channel)
    ...
    for plot_key in plot_types:
        ...
        for era in era_list:
            ...
            for input_channel in input_channels:
                ...
                hist_file = load_or_create_channel_hists(era, input_channel, scores_region)
...
if __name__ == "__main__":
    ...
    def get_background_processes():
        ...
        if is_combined_era or is_combined_channel:
            ...
            sample_dir = (
                f"{WORKDIR}/SignalRegionStudyV3/samples/{reference_era}/"
    ...
    for sample_channel, region_label, show_data in regions:
        ...
        # =================================================================
        sample_dir = f"{WORKDIR}/SignalRegionStudyV3/samples/{args.era}/{sample_channel}/{args.masspoint}"
