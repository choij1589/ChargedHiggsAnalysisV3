...
#
# This script orchestrates impact plot calculation across mass points. By
# default, --mode all processes only the fully combined All+Combined target.
#
# Usage:
#   # Run2 only
...
#
#   # Fully combined only (default for --mode all)
...
# Get the script directory (SignalRegionStudyV3/)
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo ""
            echo "Modes:"
            echo "  --mode all    - Process All+Combined only (default)"
            ...
            echo "  --unblind              - Use fully unblinded templates; runs all baseline/particlenet mass points"
...
# Select mass points based on method and blinding mode. Blinded impacts use the
# configured impact subset; unblinded impacts scan all baseline/particlenet
# mass points.
...
echo "============================================================"
echo "SignalRegionStudyV3 Impact Plot Batch Submission"
...
echo "  $0 --mode $MODE --method $METHOD $([[ -n "$MASSPOINT_OVERRIDE" ]] && echo --masspoint "$MASSPOINT_OVERRIDE") --binning $BINNING $([[ "$UNBLIND" == true ]] && echo --unblind) $([[ "$PARTIAL_UNBLIND" == true ]] && echo --partial-unblind) $([[ "$NUISANCE" == preserve_shape ]] && echo --nuisance preserve_shape) --plot-only"
echo "============================================================"
