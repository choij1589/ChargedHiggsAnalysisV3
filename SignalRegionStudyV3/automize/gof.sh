...
#
# Runs GoF tests across mass points. By default, --mode all submits the review
# set: All+Combined, All+SR1E2Mu, All+SR3Mu, Run2+Combined, Run3+Combined.
#
# Usage:
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo ""
            echo "Modes:"
            echo "  --mode all    - Default review set: All+Combined, All+SR1E2Mu, All+SR3Mu, Run2+Combined, Run3+Combined"
            echo "                  With --channel, process Run2, Run3, and All for that channel."
            echo "  --mode run2   - Process Run2 only, Combined unless --channel is set"
            echo "  --mode run3   - Process Run3 only, Combined unless --channel is set"
            echo ""
            echo "Single Era:"
            ...
            echo "  --masspoint LIST    - Comma-separated mass points overriding config subset"
...
# Select mass points based on method and blinding mode. If no gof subset exists
# in configs/masspoints.json, load_masspoints.sh falls back to the full
# baseline/particlenet lists.
...
echo "============================================================"
echo "SignalRegionStudyV3 GoF Batch Submission"
...
echo "Method:          $METHOD"
if [[ "$CHANNEL_OVERRIDE" == true || -n "$SINGLE_ERA" || "$MODE" != "all" ]]; then
    echo "Channel:         $CHANNEL"
else
    echo "Channel targets: All+Combined, All+SR1E2Mu, All+SR3Mu, Run2+Combined, Run3+Combined"
fi
...
process_era() {
    ...
    echo ""
    echo "============================================================"
    echo "Processing target: ${era}+${channel}"
    echo "============================================================"
    for masspoint in "${MASSPOINTs[@]}"; do
        run_gof_single "$era" "$channel" "$masspoint"
    done
}

process_default_all_targets() {
    process_era "All" "Combined"
    process_era "All" "SR1E2Mu"
    process_era "All" "SR3Mu"
    process_era "Run2" "Combined"
    process_era "Run3" "Combined"
}
...
case "$MODE" in
    run2)
        process_era "Run2" "$CHANNEL"
        ;;
    run3)
        process_era "Run3" "$CHANNEL"
        ;;
    all)
        if [[ "$CHANNEL_OVERRIDE" == true ]]; then
            process_era "Run2" "$CHANNEL"
            process_era "Run3" "$CHANNEL"
            process_era "All" "$CHANNEL"
        else
            process_default_all_targets
        fi
        ;;
...
echo "  $0 --mode $MODE $([[ "$CHANNEL_OVERRIDE" == true ]] && echo --channel "$CHANNEL") --method $METHOD $([[ -n "$MASSPOINT_OVERRIDE" ]] && echo --masspoints "$MASSPOINT_OVERRIDE") --binning $BINNING $([[ "$UNBLIND" == true ]] && echo --unblind) $([[ "$PARTIAL_UNBLIND" == true ]] && echo --partial-unblind) $([[ "$NUISANCE" == preserve_shape ]] && echo --nuisance preserve_shape) --plot-only"
echo "============================================================"
