...
def has_bjet_in_event(node_features: torch.Tensor, separate_bjets: bool = False,
                     btag_threshold: float = 0.7) -> bool:
    """
    Check if an event contains b-jets based on node features.

    Args:
        node_features: Node feature tensor (num_nodes, num_features)
        separate_bjets: If True, checks isBjet flag (index 8)
                       If False, checks btag score (index 5) for jets
        btag_threshold: Threshold for btag score (default: 0.7 for medium WP)
    ...
    if separate_bjets:
        ...
        has_bjet = (node_features[:, 8] > 0.5).any().item()
    else:
        # Node features: [E, Px, Py, Pz, charge, btagScore, isMuon, isElectron, isJet]
        # Check btag score (index 5) for jets (index 8)
        is_jet = node_features[:, 8] > 0.5
        btag_score = node_features[:, 5]
        has_bjet = (is_jet & (btag_score > btag_threshold)).any().item()

    return has_bjet


def filter_bjet_events(data_list: List[Data], separate_bjets: bool = False,
                      btag_threshold: float = 0.7) -> Tuple[List[Data], int, int]:
    """
    Filter a list of graph data to only events containing b-jets.

    Args:
        data_list: List of PyTorch Geometric Data objects
        separate_bjets: Whether b-jets are separate node types
        btag_threshold: Threshold for btag score
    ...
    for data in data_list:
        if has_bjet_in_event(data.x, separate_bjets, btag_threshold):
...
def create_bjet_subset_loader(data_list: List[Data], separate_bjets: bool = False,
                              btag_threshold: float = 0.7,
    ...
    """
    # Filter events
    filtered_data, original_count, filtered_count = filter_bjet_events(
        data_list, separate_bjets, btag_threshold
    )
...
def get_bjet_subset_statistics(data_list: List[Data], separate_bjets: bool = False,
                               btag_threshold: float = 0.7) -> dict:
    """
    ...
    for data in data_list:
        if has_bjet_in_event(data.x, separate_bjets, btag_threshold):
            bjet_events += 1
            # Count number of b-jets in this event
            if separate_bjets:
                num_bjets = (data.x[:, 8] > 0.5).sum().item()
            else:
                is_jet = data.x[:, 8] > 0.5
                btag_score = data.x[:, 5]
                num_bjets = (is_jet & (btag_score > btag_threshold)).sum().item()
