...
parser.add_argument("--separate_bjets", action="store_true", default=False, help="separate b-jets as distinct particles")
...
dataset_stats = {
    ...
    "separate_bjets": args.separate_bjets,
    "eras": {},
    "folds": {},
    "total_events": 0,
    "total_weight": 0.0
...
for era in ["2016preVFP", "2016postVFP", "2017", "2018"]:
    ...
    for channel in channels_to_process:
        ...
        if os.path.exists(file_path):
            ...
            # Convert events to graph data (no labels, with weights)
...
dataset_dir = "dataset_bjets" if args.separate_bjets else "dataset"
if args.sample_type == "signal":
    baseDir = f"{WORKDIR}/ParticleNet/{dataset_dir}/samples/signals/{args.sample}"
else:
    baseDir = f"{WORKDIR}/ParticleNet/{dataset_dir}/samples/backgrounds/{args.sample}"
