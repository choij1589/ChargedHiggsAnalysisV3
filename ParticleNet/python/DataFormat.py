...
def getJets(evt, separate_bjets=False):
    jets = []
    bjets = []
    ...
    for pt, eta, phi, mass, charge, btagScore, isBtagged in jets_zip:
        if separate_bjets and isBtagged:
    ...
    # For backward compatibility, if not separating bjets,
    # also return bjets as filtered list from jets
    if not separate_bjets:
